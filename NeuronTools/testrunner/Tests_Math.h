...
TEST_SUITE(Math)
{
    ...
    TEST_CASE(Dot) {
        ...
        CHECK(std::fabs(d) < 1e-6f); // perpendicular → 0
    });
    ...
    TEST_CASE(Cross) {
        ...
        CHECK(std::fabs(Neuron::Math::GetX(z))        < 1e-6f);
        CHECK(std::fabs(Neuron::Math::GetY(z))        < 1e-6f);
        CHECK(std::fabs(Neuron::Math::GetZ(z) - 1.0f) < 1e-6f);
    });

    TEST_CASE(LengthSquare) {
        XMVECTOR v = Neuron::Math::Set(1.0f, 2.0f, 2.0f);
        CHECK(std::fabs(Neuron::Math::LengthSquare(v) - 9.0f) < 1e-6f);
        CHECK(std::fabs(Neuron::Math::Length(v)       - 3.0f) < 1e-6f);
    });

    TEST_CASE(SetLength) {
        XMVECTOR v = Neuron::Math::Set(1.0f, 0.0f, 0.0f);
        XMVECTOR s = Neuron::Math::SetLength(v, 5.0f);
        CHECK(std::fabs(Neuron::Math::Length(s) - 5.0f) < 1e-5f);
    });

    TEST_CASE(SmoothStep01) {
        CHECK(std::fabs(Neuron::Math::SmoothStep01(0.0f))        < 1e-6f);
        CHECK(std::fabs(Neuron::Math::SmoothStep01(1.0f) - 1.0f) < 1e-6f);
        // Mid-point: smooth-step(0.5) = 0.5
        CHECK(std::fabs(Neuron::Math::SmoothStep01(0.5f) - 0.5f) < 1e-6f);
        // Clamped outside [0,1]
        CHECK(std::fabs(Neuron::Math::SmoothStep01(-1.0f))        < 1e-6f);
        CHECK(std::fabs(Neuron::Math::SmoothStep01(2.0f) - 1.0f)  < 1e-6f);
    });
