...
int main()
{
    ...
    // pin it (§8.3). Real clients ship with this key baked in.
    {
        ...
        if (f) f.write(reinterpret_cast<const char*>(pub.data()),
                       static_cast<std::streamsize>(pub.size()));
    ...
    Neuron::Net::ServerHost  host(&crypto,
                                  std::vector<uint8_t>(crypto.GetStaticPrivateBlob().begin(),
    ...
    while (g_running) {
        ...
        while (acc.ConsumeStep()) {
            world.Step(static_cast<float>(Neuron::Sim::kSimDeltaSeconds));
            stepped = true;
        }
