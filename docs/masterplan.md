...
```
**In progress (M2):** `NeuronAudio/` (XAudio2/X3DAudio client lib + `NeuronAudioTest`, §11.3) is scaffolded — voice graph / mixer / spatializer / WAV reader landed; buffer-queue streaming, cue catalog and the Windows device smoke test still pending. `NeuronTools/` exists with the platform-independent asset parsers (`WavParse`/`DdsParse`/`CmoParse`/`FontAtlasLayout`) + a Linux `testrunner`; the `datacook`/`datacheck` + asset-cooker executables (§12.6) are still to come. Both follow the flat-files + VS Filters convention.
