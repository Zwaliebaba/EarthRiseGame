...
| **F** Canvas HUD + radar | 🟡 in progress | **MU-1 done:** `CanvasRenderer` extended to textured + font primitives (UV vertex, solid/textured-linear/textured-point modes via a root constant, SRV heap + linear/point samplers, batch-per-texture); real bitmap **`DrawText`** (EditorFont atlas via `FontAtlasLayout`), `DrawTexturedQuad`, `DrawTriangle`. The **Main Menu window** (grey/red gradient chrome + title bar + close box + skinned buttons) renders from the reference. **Next (MU-2/3):** widget toolkit (Window/Button/DropDown/Label, hit-test/z-order), pointer interactivity, the other 4 windows, radar disc. |
