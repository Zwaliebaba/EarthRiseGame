...
| **D** Particles | 🟡 in progress | `ParticleRenderer` — CPU-simulated drifting "space dust" field (follows the camera focus, wraps in a box) rendered as camera-facing **additive billboards** sampling `Particle.dds` into the HDR target (so they bloom). Depth-tested, no depth-write. **Remaining:** move sim to GPU compute (masterplan §11.2), per-entity emitters (thrusters/impacts/warp). |
...
| **F** Canvas HUD + radar | 🟢 mostly done | `CanvasRenderer` does textured + condensed-bitmap-font primitives (DrawText/DrawTexturedQuad/DrawTriangle/DrawVGradient), per-frame VB (no flicker). **Windowed UI (MU-1→3):** Main Menu + Options + Screen/Graphics/Other Options panels, faithful to the Darwinia Window/Button/TextRenderer source. **Interactive** — pointer hover/press/click, draggable title bars, close boxes, and working **DropDown** widgets (popup list + select) + **Label** rows (live FPS / build). Layout/hit-test in `UiLayout.h` (Linux-tested). **Remaining:** radar disc, a proper StringTable (§22.4) + z-order raising, and wiring settings to real engine knobs (area G). |
