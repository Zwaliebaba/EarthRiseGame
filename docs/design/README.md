...
| [`review/directxmath-dx12-mmo-validation.md`](review/directxmath-dx12-mmo-validation.md) | **Code validation: DirectXMath adoption, DirectX 12 performance, MMO multithreading readiness** — current tree after M3 + M4 areas A–C; alignment-correct DXMath use, idiomatic single-threaded DX12, and the threading blockers (M4 interest/delta path built-but-unwired, IOCP compiled-but-unused, non-thread-safe per-connection/ECS state) with a prioritized fix order toward M4 | pending (validation map for M4) |
