...
The platform-independent asset parsers live in their **owning** libraries — `WavParse.h` in `NeuronAudio/`, and `DdsParse.h`/`CmoParse.h`/`FontAtlasLayout.h` in `NeuronRender/` — kept dependency-free so the runtime loaders and any build-time validators share one implementation. Outside the solution, `NeuronTools/` holds **only** `testrunner/` — a dependency-free harness (built with `make`) that `#include`s those parser headers from their owning libraries and exercises them on Linux CI without a Windows build (plus future build-time `*check`/cook tools). **Nothing depends on `NeuronTools`** — it references the libraries, never the reverse, so it can be removed once the checks run natively on Windows.
